...
class ShapeLearner(object):
    def __init__(self, conf, inference=False):
        ...
        self.ds = ShapeDataSet(prob_bkg=conf.bkg_prob, no_bkg=conf.no_bkg,
                                     shape_only=conf.shape_only, color_only=conf.color_only,
                                     n_shapes=conf.n_shapes, n_colors=conf.n_colors)
    ...
    def train(self, conf, epochs, save_final=True):
        ...
        for e in epoch_iter:
            ...
            # train
            self.loader.dataset.set_mode('train')  # todo check this works :)
            for imgs, labels in tqdm(self.loader, desc='epoch {}'.format(e), total=len(self.loader), position=0):
                imgs = imgs.to(conf.device, dtype=torch.half if conf.half else None)
